...
func (s *Dbase) ReadLocalUsers(path string) error {
	if path == "" {
		fmt.Println("Файл хранилища отключен, загрузка пропущена")
		return nil
	}
	...
	// Загружаем данные в память
	for _, entry := range db {
		s.DataUsers[entry.ID] = entry.User
	}

	fmt.Println("Данные успешно загружены из файла")
	fmt.Println(s.DataUsers)
	return nil
}

// WriteLocal сохраняет данные в файл JSON
func (s *Dbase) WriteLocalUsers(path string) error {
	if path == "" {
		fmt.Println("Файл хранилища отключен, сохранение пропущено")
		return nil
	}
	...
	// Преобразуем данные в срез структур LocalLink
	var db []models.UsersLink
	for id, url := range s.DataUsers {
		db = append(db, models.UsersLink{ID: id, User: url})
	}
