...
//		adr := "http://localhost:8080/"
//		server := New(&db, adr, host, nil)
//		server.database.SaveLink("https://ru.wikipedia.org/wiki/SHA-1", "b12a6809")
//		server.database.SaveLink("https://github.com/Ippolid/shortLink/pulls?q=is%3Apr+is%3Aopen", "14603b1d")
//
//		r := server.newServer()
//		ts := httptest.NewServer(r)
//		defer ts.Close()
...
func TestCreateLink(t *testing.T) {
	...
	shortLink := strings.TrimSpace(resp.String())
	assert.Contains(t, shortLink, "http://localhost:8080/")

	resp2, err := client.R().Get(shortLink)
	require.NoError(t, err)
	assert.Equal(t, http.StatusTemporaryRedirect, resp2.StatusCode())
	assert.Equal(t, "https://ru.wikipedia.org/wiki/SHA-1", resp2.Header().Get("Location"))
}
...
func TestGetLink(t *testing.T) {
	...
	// Создаётся короткая ссылка вручную
	resp, err := client.R().SetBody("https://github.com/Ippolid/test").Post(ts.URL + "/")
	require.NoError(t, err)
	shortLink := strings.TrimSpace(resp.String())
	...
	require.NoError(t, err)
	assert.Equal(t, http.StatusTemporaryRedirect, resp2.StatusCode())
	assert.Equal(t, "https://github.com/Ippolid/test", resp2.Header().Get("Location"))

	// Проверка несуществующего
	resp3, err := client.R().Get(ts.URL + "/unknown-id")
	require.NoError(t, err)
...
func TestCreateLinkApi(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, http.StatusCreated, resp.StatusCode())
	assert.Contains(t, resp.String(), `"result":"http://localhost:8080/`)
}
